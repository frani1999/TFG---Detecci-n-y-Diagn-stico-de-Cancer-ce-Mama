...
class CBISDDSM():
    def __init__(self, data_path):
        ...
        self.abnormality = self.data['abnormality']
        self.labels = self.abnormality
        self.image_folder = self.data['image_folder']
        self.cropped_image_name = self.data['image_name']

    def read_images(self):
        self.images = []
        for i in range(len(self.cropped_image_name)):
            #print(self.cropped_image_name[i])
            img = cv2.imread("T:/fbr/CBIS-DDSM/jpeg/" + self.image_folder[i] + "/" + self.cropped_image_name[i], 0)
...
#Path to train data:
path_train = "train_mass_or_calc.csv"
#Path to test data:
path_test = "test_mass_or_calc.csv"
