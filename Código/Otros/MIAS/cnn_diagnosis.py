...
path_to_labels = "C:/Users/RDuser-E1/Desktop/TFG/all-mias/info/MIAS.csv"

#PRE-PROCESADO DE LOS DATOS:

#Leemos los datos de nuestro csv:
data = pd.read_csv(path_to_labels, sep=",")

#Borramos los casos normales. Nos quedamos con los anómalos:
data.dropna(subset = ["severity"], inplace=True)
data.reset_index(inplace = True)
#Elimina caso 5 que está repetido:
data = data.drop([3], axis=0)
data.reset_index(inplace = True)
#Borramos primera columna que es índice:
data = data.drop('level_0', axis =1)
data = data.drop('index', axis =1)

#Transforma columna severity para entrenar:
# B = 0
# M = 1
lb = LabelEncoder()
data['severity'] = lb.fit_transform(data['severity'])

#Extraigo la información de columna severity y la guardo en un numpy array: ETIQUETAS
label = data.severity
...
for i in range(len(label)):
        img_name.append(path_to_images + data.reference[i]+ '.pgm')
...
x_train = np.reshape(x_train, (x_train.shape[0],224, 224, 1)) # 1 for gray scale 32940
x_test = np.reshape(x_test, (x_test.shape[0],224, 224,1)) # 10980
