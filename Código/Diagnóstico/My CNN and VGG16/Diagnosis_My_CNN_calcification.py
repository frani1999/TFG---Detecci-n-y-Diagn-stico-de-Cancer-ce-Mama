...
class CBISDDSM():
    def __init__(self, data_path):
        ...
        self.pathology = self.data['pathology']
        self.labels = self.pathology.replace("BENIGN_WITHOUT_CALLBACK", "BENIGN")
        self.cropped_image_path = self.data['cropped image file path']

    def read_images(self):
        self.images = []
        cnt_tissue = 0
        for i in range(len(self.cropped_image_path)):
            #print(self.cropped_image_path[i])
            names_images = os.listdir("T:/fbr/CBIS-DDSM/jpeg/" + self.cropped_image_path[i])
            #print(names_images[0])
            img = cv2.imread("T:/fbr/CBIS-DDSM/jpeg/" + self.cropped_image_path[i] + "/" + names_images[0], 0)
            #Pre_process:
            img_resize = cv2.resize(img, (224, 224))
            # Convert into grayscale:
            #gray_img = cv2.cvtColor(img_resize, cv2.COLOR_BGR2GRAY)
            # Contrast Limited Adaptive Histogram Equalization:
            #clahe = cv2.createCLAHE(clipLimit=40)  # sets the threshold for contrast limiting
            #gray_img_clahe = clahe.apply(gray_img)
            # Equalize hist:
            #gray_img_eq_hist = cv2.equalizeHist(gray_img_clahe)
            # Apply threshold:
            #ret, o1 = cv2.threshold(gray_img_eq_hist, 180, 255, cv2.THRESH_TOZERO)  # THRESH_BINARY THRESH_TOZERO
            # Normalization
            #o1_norm = o1 / 255
...
#Path to train data:
path_train = "calc_case_description_train_set_equitable.csv"
#Path to test data:
path_test = "calc_case_description_test_set_equitable.csv"
...
opt = tf.keras.optimizers.Adam(learning_rate = 0.000005) # momentum = 0.9, decay = 0.0
